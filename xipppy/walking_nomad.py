'''
This is a Xipppy test script that will run through the following operations:
...
4.5) Configure fast settle settings on each front end
...
if __name__== '__main__':
    '''If processor is connected with UDP, rewrite line 39 as "with xp.xipppy_open(use_tcp=False)"'''
...
#3      '''Configure electrode stim FE to have stim data streams active, all hi-rest streams to active, and deactivate all other streams'''
        if len(stimChans): 
            for jj in stimChans:
                ...
                time.sleep(0.001) 
                #pause has to be used bc property-changing fxns are asynchronous and messy
            for kk in range(0,len(stimFEs)):
                ...
                #xp.signal_set(stimChans[num_elec*kk].item(),'lfp',False) #commenting out fixes huge artifact in the lfp channel
                time.sleep(0.001)
        if len(recChans):
